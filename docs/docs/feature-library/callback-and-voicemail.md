...
#### Custom Audio-Based Wait Experience (_wait-experience-custom_)

A parallel implementation, `wait-experience-custom.protected.js`, is also provided alongside the original `wait-experience` function. It offers the same callback capability but plays pre-recorded audio prompts (via `<Play>`) instead of Twilio TTS (`<Say>`) for all static prompts, and drops the voicemail option in favor of a simpler "press 1 for a callback, or continue holding" menu with no initial "press star" gate. It also announces an estimated wait time - computed from [Workflow Cumulative Statistics](https://www.twilio.com/docs/taskrouter/api/cumulative-statistics) (`avgTaskAcceptanceTime`) - between two of the recorded prompts.

To use it instead of the original, set its URL as the **_Hold Music URL_** / `waitUrl` instead:

`https://custom-flex-extensions-serverless-XXXX-dev.twil.io/features/callback-and-voicemail/studio/wait-experience-custom`

Before deploying, add the actual `.wav` recordings to `serverless-functions/src/assets/features/callback-and-voicemail/` - the function ships with empty placeholder files (see the `audio` map in `wait-experience-custom.protected.js` for the expected filenames), including a `digits/` subfolder with one recording per digit (`0.wav` - `9.wav`) used to read back an entered phone number digit-by-digit via `<Play>`. `<Say>` is only used for the estimated wait time in minutes (just the number - the surrounding "minutos" wording is part of the recorded prompt that follows it). The same confirmation prompt (`callback-confirmed.wav`) is played on successful callback submission regardless of whether the caller kept their own number or entered a different one.

**Per-TaskQueue hold audio:** While the caller holds (nested in the `<Gather>` after the callback-offer menu), the function plays audio selected by the friendly name of the TaskRouter TaskQueue their call is queued on (`task.taskQueueFriendlyName`), instead of a single generic hold track. The mapping is hardcoded in the `queueHoldAudio` object at the top of `wait-experience-custom.protected.js` - add an entry per TaskQueue friendly name (an array of one or more filenames, played in order), and update `defaultHoldAudio` for any queue not explicitly listed. These filenames are resolved against the audio base URL (see below), not the `features/callback-and-voicemail/` asset subfolder.

**Configurable audio base URL:** By default, all `<Play>` URLs are built from this Serverless service's own domain (`https://${context.DOMAIN_NAME}`). To host audio files elsewhere (e.g. a CDN) without a code deploy, set `custom_audio_base_url` under this feature's block in Flex Service Configuration - editable via the Flex Admin UI's **Callback and Voicemail** feature card (Admin UI's config editor is a generic JSON form, so no extra UI work is needed to expose this field). The function caches the resolved value for 5 minutes per warm container to avoid calling the Flex Configuration API on every wait-loop tick.
